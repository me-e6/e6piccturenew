...
class SearchControllers extends ChangeNotifier {
  ...
  // ------------------------------------------------------------
  // LOAD SEARCH - Initialize with user ID
  // ------------------------------------------------------------
  Future<void> loadSearch(String? userId) async {
    _currentUserId = userId ?? _auth.currentUser?.uid;

    // Optional: Pre-load some suggested users or recent searches
    if (_currentUserId != null) {
      isLoading = true;
      notifyListeners();

      try {
        // You can load suggested users here if needed
        // final suggested = await _userService.getSuggestedUsers(_currentUserId);
        // results = suggested;
      } catch (e) {
